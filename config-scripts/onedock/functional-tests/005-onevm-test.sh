...
# Check pending state
while sudo lxc-attach -n onetest -- onevm list | grep "container-img" | awk '{if($5 == "pend"){ exit 0 } else { exit 1 }}'
do
    sleep 2
done
...
# Check running state
sudo lxc-attach -n onetest -- onevm list | grep "container-img" | awk '{if($5 == "runn"){ exit 0 } else { exit 1 }}'
